...
use burst_node::Node;
use burst_rpc_messages::{RpcCommand, RpcError, StatsType};
use burst_types::{Account, AccountInfo, BlockHash, SavedBlock};
...
impl RpcCommandHandler {
    ...
    fn call_handler(&self, command: RpcCommand) -> anyhow::Result<serde_json::Value> {
        ...
        let response = match command {
            ...
            // Burst-specific RPC commands
            RpcCommand::BrnBalance(args) => to_value(self.brn_balance(args)?),
            RpcCommand::RequestVerification(args) => to_value(self.request_verification(args)?),
            RpcCommand::VoteVerification(args) => to_value(self.vote_verification(args)?),
            RpcCommand::TrstHistory(args) => to_value(self.trst_history(args)?),
